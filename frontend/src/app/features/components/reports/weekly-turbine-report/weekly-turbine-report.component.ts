...
import { WeeklyTurbineReportDto } from '../reports';

@Component({
  selector: 'app-weekly-turbine-report',
  templateUrl: './weekly-turbine-report.component.html',
  standalone: true,
  imports: [MatCardModule, MatIconModule, MatTableModule, MatPaginatorModule],
})
export class WeeklyTurbineReportComponent {
  @Input() turbineId!: string;

  public reports: WeeklyTurbineReportDto[] = [];
  ...
  private getReport(projectId: string) {
    this.activityService
      .getWeeklyTurbineReports(projectId)
      .subscribe((response) => {
